...
capabilities:
  - Svelte 5 runes ($state, $derived, $effect, $props) and fine-grained reactivity
  - SvelteKit 2 file-based routing, load functions, form actions, SSR/SSG/SPA
  - Reactive state management with Svelte stores and runes
  - Component patterns with snippets, events, and slots
  - UI integration with Tailwind CSS, DaisyUI, Skeleton UI
...
auto_activate:
  keywords: ["svelte", "sveltekit", "runes", "$state", "$derived", "$effect", "svelte-kit"]
  conditions: ["Svelte component development", "SvelteKit feature implementation", "Svelte UI development tasks", "Figma to Svelte implementation"]
